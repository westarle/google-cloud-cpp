...
TEST_F(GoogleCredentialsTest, QuotaProjectFromCredentialsFile) {
  ...
  "refresh_token": "1/test-test-test",
  "type": "authorized_user",
  "quota_project_id": "project-from-file"
})***";
  auto const env = ScopedEnvironment(GoogleAdcEnvVar(), filename.c_str());
  ...
  EXPECT_CALL(std::move(*mock_response), ExtractPayload)
      .WillOnce(Return(ByMove(MakeMockHttpPayloadSuccess(
          std::string(R"({"token_type": "Type", "access_token": "access-token-value", "expires_in": 1234})")))));
  using FormDataType = std::vector<std::pair<std::string, std::string>>;
  EXPECT_CALL(*mock_client, Post(_, _, ::testing::An<FormDataType const&>()))
      .WillOnce(Return(ByMove(
          std::unique_ptr<rest_internal::RestResponse>(std::move(mock_response)))));
  ...
  auto headers = (*creds)->AuthenticationHeaders(
      std::chrono::system_clock::now(), "https://foo.googleapis.com");
...
TEST_F(GoogleCredentialsTest, QuotaProjectEnvVarOverridesFile) {
  using ::google::cloud::testing_util::MakeMockHttpPayloadSuccess;
  using ::google::cloud::testing_util::MockRestResponse;
  using ::testing::ByMove;
  using ::testing::Return;
  ...
  auto const env = ScopedEnvironment(GoogleAdcEnvVar(), filename.c_str());
  auto const env_quota = ScopedEnvironment("GOOGLE_CLOUD_QUOTA_PROJECT", "project-from-env");

  auto opts = google::cloud::internal::PopulateCommonOptions(Options{}, {}, {}, {}, "default.googleapis.com");
  EXPECT_EQ(opts.get<UserProjectOption>(), "project-from-env");
  ...
  auto creds =
      GoogleDefaultCredentials(std::move(opts), client_factory.AsStdFunction());
  (void)std::remove(filename.c_str());
  ASSERT_STATUS_OK(creds);
  ...
  auto headers = (*creds)->AuthenticationHeaders(
      std::chrono::system_clock::now(), "https://foo.googleapis.com");
  ASSERT_STATUS_OK(headers);
  EXPECT_THAT(*headers, Not(::testing::Contains(rest_internal::HttpHeader{
                            "x-goog-user-project", "project-from-file"})));
}

TEST_F(GoogleCredentialsTest, QuotaProjectFromServiceAccountFile) {
  using ::google::cloud::testing_util::MakeMockHttpPayloadSuccess;
  using ::google::cloud::testing_util::MockRestResponse;
  using ::testing::ByMove;
  using ::testing::Return;
